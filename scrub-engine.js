/* ============================================================================
   ...
   USAGE
     mountScrollWorld(document.getElementById('world'), {
       ...
       sections: [
         { id, label, still, stillMobile, clip, clipMobile, clipStart, clipEnd, accent,
           // clipStart / clipEnd optionally map this scene to a time range inside
           // a longer source video, so one film can carry several story beats.
...
function mountScrollWorld(container, config) {
  ...
  // The desktop loop can follow high-refresh displays while constrained devices use
  // a lighter cadence. requestAnimationFrame remains the real clock, so a 60/90/120 Hz
  // panel naturally runs at its own native ceiling rather than fabricating frames.
  const RENDER_FPS = Math.min(144, Math.max(30, config.renderFPS || 60));
  const MOBILE_RENDER_FPS = Math.min(RENDER_FPS, Math.max(30, config.mobileRenderFPS || 60));
  const SOURCE_FPS = Math.max(1, config.sourceFPS || 24);
  const connection = navigator.connection || navigator.mozConnection || navigator.webkitConnection;
  const saveData = !!(connection && connection.saveData);
  const lowPower = saveData ||
    (navigator.hardwareConcurrency && navigator.hardwareConcurrency <= 4) ||
    (navigator.deviceMemory && navigator.deviceMemory <= 4);
  const activeRenderFPS = () => (isMobile() || lowPower) ? MOBILE_RENDER_FPS : RENDER_FPS;
  const motionBlurEnabled = config.motionBlur !== false && !reduce && !saveData;
  const motionBlurMax = Math.max(0, config.motionBlurMax == null ? 1.65 : config.motionBlurMax);
  const N = SECTIONS.length;
  if (!N) return;

  injectCSS();
  container.classList.add('sw-root');
  container.dataset.renderFps = String(RENDER_FPS);
  container.dataset.activeRenderFps = String(activeRenderFPS());
  container.dataset.sourceFps = String(SOURCE_FPS);
  ...
  const studioGrid = el('div', 'sw-studio-grid');
  const worldTitle = el('div', 'sw-worldtitle');
  worldTitle.setAttribute('aria-hidden', 'true');
  worldTitle.textContent = (SECTIONS[0].label || '').toUpperCase();
  const studioMeta = el('div', 'sw-studio-meta');
  studioMeta.setAttribute('aria-hidden', 'true');
  (config.meta || []).slice(0, 2).forEach(value => {
    const item = el('span'); item.textContent = value; studioMeta.appendChild(item);
  });
  ...
  [sky, scrollbar, topbar, stage, studioGrid, worldTitle, studioMeta, copylayer, route, hint, track]
  ...
  SECTIONS.forEach((s, i) => {
    ...
    c.innerHTML =
      ...
      (s.titleHtml ? `<h2 class="sw-copy__title">${s.titleHtml}</h2>`
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = copies[i];
      c.style.opacity = cop;
      c.style.filter = reduce ? 'none' : `blur(${((1 - cop) * 7).toFixed(2)}px)`;
      // Keep desktop copy optically centred while adding the subtle scroll drift.
      // The original transform replaced translateY(-50%), which pushed longer
      // portfolio copy below the viewport at its peak opacity.
      c.style.transform = reduce ? 'translateY(-50%)'
    ...
    if (near !== activeIndex) {
      activeIndex = near;
      container.dataset.section = SECTIONS[near].id || '';
      worldTitle.textContent = (SECTIONS[near].label || '').toUpperCase();
      dots.forEach((d, k) => d.classList.toggle('is-active', k === near));
      nav.querySelectorAll('.sw-nav__item').forEach((n, k) => n.classList.toggle('is-active', k === near));
      container.style.setProperty('--sw-accent', SECTIONS[near].accent || '');
    }
    scrollbarFill.style.transform = `scaleX(${clamp(y / (totalW * vh))})`;
    container.style.setProperty('--sw-grid-y', `${(-y * 0.018).toFixed(2)}px`);
    hint.style.opacity = clamp(1 - y / (0.5 * vh));
    if (particles) particles.style.transform = `translate3d(0, ${-y * 0.05}px, 0)`;
    ticking = false;
  }

  let lastRenderAt = 0, lastMotionY = scrollPosition, motionBlur = 0;
  function raf(now) {
    requestAnimationFrame(raf);
    if (document.hidden) return;
    const activeFPS = activeRenderFPS();
    const frameMs = 1000 / activeFPS;
    const elapsed = lastRenderAt ? now - lastRenderAt : frameMs;
    if (elapsed < frameMs - 0.5) return;
    const dt = Math.min(elapsed, frameMs * 4);
    lastRenderAt = now - (elapsed % frameMs);
    container.dataset.activeRenderFps = String(activeFPS);

    // UI interpolation may run up to 144 Hz, while the current films remain 24fps.
    // Seeking more finely than half a source frame only resolves the same frame and
    // wastes decoder time, so video work stays bounded even on a fast display.
    const eps = 1 / (SOURCE_FPS * 2);
    const baseFollow = isMobile() ? 0.2 : 0.26;
    const follow = 1 - Math.pow(1 - baseFollow, dt / (1000 / 60));

    // Short, velocity-reactive blur sells momentum without leaving the stationary
    // frames soft. Phones/low-power devices get a much smaller cap; reduced-motion
    // and data-saver users get no blur at all.
    const speed = Math.abs(scrollPosition - lastMotionY) / Math.max(1, dt);
    lastMotionY = scrollPosition;
    const deviceBlurMax = (isMobile() || lowPower) ? Math.min(0.6, motionBlurMax) : motionBlurMax;
    const blurTarget = motionBlurEnabled ? Math.min(deviceBlurMax, Math.max(0, (speed - 0.08) * 0.24)) : 0;
    const blurFollow = 1 - Math.pow(1 - (blurTarget > motionBlur ? 0.42 : 0.16), dt / (1000 / 60));
    motionBlur += (blurTarget - motionBlur) * blurFollow;
    if (motionBlur < 0.015) motionBlur = 0;
    container.style.setProperty('--sw-motion-blur', `${motionBlur.toFixed(2)}px`);
    container.classList.toggle('is-scrolling', motionBlur > 0.03);
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      if (!s.hasClip || !s.ready || !s.video) continue;
      // Loaded off-screen segments used to keep chasing their targets, so several
      ...
      const t = rangeStart + clamp(s.cur, 0, 0.999) * rangeDuration;
